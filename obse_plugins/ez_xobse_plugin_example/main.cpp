...
namespace
{
	...
	std::string EscapeCSVField(const char* input)
	{
	...
	void ExportRevoiceCsvForActivePlugin()
	{
		...
		for (tList<TESTopic>::Iterator topicIt = handler->topics.Begin(); !topicIt.End() && topicIt.Get(); ++topicIt)
		{
			...
			for (TESTopic::QuestInfoEntry* qEntry = topic->questInfoList; qEntry; qEntry = qEntry->next)
			{
				...
				TESQuest* quest = qEntry->data->parentQuest;
				...
				for (UInt32 i = 0; i < qEntry->data->infoList.numObjs; ++i)
				{
					...
					for (tList<EditorResponseData>::Iterator rIt = info->responseList.Begin(); !rIt.End() && rIt.Get(); ++rIt)
					{
						...
						const std::string outPath = BuildRevoiceOutputPath(info, topic, quest, response, ctx);
						if (outPath.empty()) {
							++skipped;
							continue;
						}
						...
						out << EscapeCSVField(formIDBuffer)
							<< ',' << EscapeCSVField(ctx.voiceID.c_str())
							<< ',' << EscapeCSVField(ctx.speakerInfo.c_str())
							<< ',' << EscapeCSVField(outPath.c_str())
							<< ',' << EscapeCSVField(text)
							<< "\n";
						++exported;
					}
		...
		out.flush();
		if (out.fail()) {
			MessageBoxA(g_editorMainWindow, "reVoice CSV export failed while finalizing output file.", "Export reVoice CSV", MB_OK | MB_ICONERROR);
			return;
		}

		std::ostringstream ss;
		ss << "reVoice CSV export complete.\n\nExported rows: " << exported << "\nSkipped rows: " << skipped << "\nOutput: " << filePath;
